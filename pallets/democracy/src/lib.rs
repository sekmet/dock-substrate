...
//! FUNCTIONS PUBLIC AND (POSSIBLY) REMOVING CODE TO MAKE THE WASM SMALLER. LOGIC CHANGES WILL BE DONE IN ANOTHER
...
// TODO: Add method to cancel proposal from master
...
impl<T: Trait> Module<T> {
	...
	pub fn try_vote(who: &T::AccountId, ref_index: ReferendumIndex, vote: AccountVote<BalanceOf<T>>) -> DispatchResult {
	...
	pub fn try_remove_vote(who: &T::AccountId, ref_index: ReferendumIndex, scope: UnvoteScope) -> DispatchResult {
		let info = ReferendumInfoOf::<T>::get(ref_index);
		VotingOf::<T>::try_mutate(who, |voting| -> DispatchResult {
